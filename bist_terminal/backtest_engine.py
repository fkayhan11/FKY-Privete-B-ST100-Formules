...
class FiscographAlphaStrategy(bt.Strategy):
    params = dict(
        ...
        use_sector_fundamentals=True,
        require_sector_leader=False,
    )
    ...
    def __init__(self):
        ...
        for data in self.stock_datas:
            ...
            self.sma50[data] = bt.indicators.SimpleMovingAverage(data.close, period=50)
    ...
    def point_in_time_fundamental_score(self, fundamentals: Dict[str, Any], symbol: str) -> Optional[float]:
        if self.p.use_sector_fundamentals:
            from signals_core import fundamental_raw_score
            sector = self.sector_map.get(symbol, "Unknown")
            return fundamental_raw_score(fundamentals, sector)
    ...
    def inverse_volatility_position_size(self, data, close: float) -> int:
        ...
        if self.p.use_dynamic_stop:
            from signals_core import dynamic_stop_multiplier
            atr_val_reg = float(self.atr[data][0])
            atr_pct = (atr_val_reg / close) * 100.0 if (math.isfinite(atr_val_reg) and close > 0) else 3.0
            mult = dynamic_stop_multiplier(atr_pct, base_multiplier=float(self.p.atr_multiplier))
        else:
            mult = float(self.p.atr_multiplier)
    ...
    def next(self):
        ...
        # Calculate historical market breadth proxy from portfolio stocks
        total_valid = 0
        above_50 = 0
        for data in self.stock_datas:
            close_val = float(data.close[0])
            sma50_val = float(self.sma50[data][0])
            if math.isfinite(close_val) and math.isfinite(sma50_val):
                total_valid += 1
        ...
        breadth_ok = True
        if total_valid > 0:
            pct_above_50 = (above_50 / total_valid) * 100.0
            if pct_above_50 < 40.0:
                breadth_ok = False
        ...
        for data in self.stock_datas:
            ...
            if position.size:
                ...
                if math.isfinite(atr_value) and atr_value > 0 and close > 0:
                    if self.p.use_dynamic_stop:
                        from signals_core import dynamic_stop_multiplier
                        ...
                        mult = float(self.p.atr_multiplier)
                    trailing_stop_price = self.highest_price[data] - (atr_value * mult)
            ...
            if isinstance(pit_fundamentals, dict):
                # Copy dict to avoid modifying cached PIT fundamentals globally
                pit_fundamentals = pit_fundamentals.copy()
                shares = to_num(pit_fundamentals.get("sharesOutstanding"))
                eps = to_num(pit_fundamentals.get("trailingEps"))
                equity = to_num(pit_fundamentals.get("equity"))
                eg = to_num(pit_fundamentals.get("earningsGrowth"))
                if shares is not None and shares > 0:
                    pit_fundamentals["marketCap"] = close * shares
                    if equity is not None and equity > 0:
                        pit_fundamentals["priceToBook"] = (close * shares) / equity
                if eps is not None and eps > 0:
                    pit_fundamentals["trailingPE"] = close / eps
                    if eg is not None and eg > 0:
                        pit_fundamentals["pegRatio"] = (close / eps) / (eg * 100.0)

            pit_score = self.point_in_time_fundamental_score(pit_fundamentals, symbol)
            if pit_score is None or pit_score <= 0:
                continue

            is_stock_uptrend = math.isfinite(trend_ma) and close > trend_ma
            
            # Pullback logic (Dynamic RSI or static)
            atr_val = float(self.atr[data][0])
            atr_pct = (atr_val / close) * 100.0 if (math.isfinite(atr_val) and close > 0) else 3.0
            
            if self.p.use_dynamic_rsi:
                from signals_core import dynamic_rsi_pullback_threshold
                rsi_threshold = dynamic_rsi_pullback_threshold(atr_pct, float(self.p.rsi_pullback_threshold))
            else:
                rsi_threshold = float(self.p.rsi_pullback_threshold)

            is_pullback = (
                (math.isfinite(pullback_ma) and close < pullback_ma)
                or (math.isfinite(rsi_value) and rsi_value < rsi_threshold)
            )

            if is_stock_uptrend and is_pullback:
                # Block buying if market breadth indicates a distribution warning
                if not breadth_ok:
                    continue
                # Check if sector leader filter is required
                if self.p.require_sector_leader:
                    zscore = self.fundamental_zscore(data)
                    symbol_sector = self.sector_map.get(normalize_symbol(data._name), "Unknown")
                    leader_threshold = self.sector_leader_thresholds.get(symbol_sector, 0.0)
                    if zscore <= leader_threshold:
                        continue # Skip if not a sector leader
                ...
                if size > 0:
                    self.log(
                        ...
                        f"sma20={pullback_ma:.2f} sma200={trend_ma:.2f} rsi={rsi_value:.2f} rsi_thresh={rsi_threshold:.1f}"
                    )
